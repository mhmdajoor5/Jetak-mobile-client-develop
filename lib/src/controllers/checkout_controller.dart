...
class CheckoutController extends CartController {
  ...
  void addOrder(List<Cart> carts) async {
    ...
    _order.deliveryFee = payment!.method == 'Pay on Pickup' ? 0 : carts[0].food?.restaurant.deliveryFee ?? 0;
  ...
  void updateCreditCard(CreditCard creditCard) {
    userRepo.setCreditCard(creditCard).then((value) {
      setState(() {});
      ScaffoldMessenger.of(scaffoldKey.currentContext!).showSnackBar(SnackBar(content: Text(S.of(state!.context).payment_card_updated_successfully)));
    });
  }
}
