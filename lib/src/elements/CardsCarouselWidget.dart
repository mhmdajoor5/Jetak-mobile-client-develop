...
class _CardsCarouselWidgetState extends State<CardsCarouselWidget> {
  ...
  Widget build(BuildContext context) {
    return (widget.restaurantsList?.isEmpty ?? true)
          ...
          child: ListView.builder(
            ...
            itemBuilder: (context, index) {
              return GestureDetector(
                onTap: () {
                  Navigator.of(context).pushNamed('/Details', arguments: RouteArgument(id: '0', param: widget.restaurantsList![index].id, heroTag: widget.heroTag));
                },
                child: CardWidget(restaurant: widget.restaurantsList![index], heroTag: widget.heroTag ?? ''),
              );
            },
          ),
        );
